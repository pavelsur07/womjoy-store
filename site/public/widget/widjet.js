...
function ISDEKWidjet(params) {
	...
	widjet.sdekWidgetEvents = function () {
		...
		.on('click', '.CDEK-widget__panel-details__back', function () {
			ipjq(this).parents('.CDEK-widget__panel-details').css('right', '-360px');
			
			if (window.innerWidth > 480) {
				ipjq(IDS.get('cdek_widget_cnt')).find(".CDEK-widget__panel-list").css('left', '30px');
			} else {
				ipjq(IDS.get('cdek_widget_cnt')).find(".CDEK-widget__panel-list").css('left', '0');
			}
